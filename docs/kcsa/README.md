# KCSA - Kubernetes and Cloud Native Security Associate

[![KCSA](https://img.shields.io/badge/CNCF-KCSA-326CE5?style=for-the-badge&logo=kubernetes&logoColor=white)](https://training.linuxfoundation.org/certification/kubernetes-and-cloud-native-security-associate-kcsa/)

The **Kubernetes and Cloud Native Security Associate (KCSA)** exam demonstrates a user's foundational knowledge and skills in security technologies in the cloud native ecosystem.

## Exam Overview

| Detail | Information |
|--------|-------------|
| **Exam Format** | Multiple Choice |
| **Number of Questions** | 60 |
| **Duration** | 90 minutes |
| **Passing Score** | 75% |
| **Certification Validity** | 3 years |
| **Cost** | $250 USD |
| **Retake Policy** | 1 free retake |

## Exam Domains & Weights

| Domain | Weight |
|--------|--------|
| [Overview of Cloud Native Security](./01-cloud-native-security-overview.md) | 14% |
| [Kubernetes Cluster Component Security](./02-cluster-component-security.md) | 22% |
| [Kubernetes Security Fundamentals](./03-kubernetes-security-fundamentals.md) | 22% |
| [Kubernetes Threat Model](./04-kubernetes-threat-model.md) | 16% |
| [Platform Security](./05-platform-security.md) | 16% |
| [Compliance and Security Frameworks](./06-compliance-security-frameworks.md) | 10% |

## Prerequisites

- Basic understanding of Kubernetes concepts (KCNA recommended)
- Familiarity with Linux command line
- General security concepts knowledge

## Study Resources

### Official Resources

- [KCSA Exam Curriculum](https://github.com/cncf/curriculum/blob/master/KCSA_Curriculum.pdf)
- [Kubernetes Security Documentation](https://kubernetes.io/docs/concepts/security/)
- [CNCF Security Whitepaper](https://github.com/cncf/tag-security/blob/main/security-whitepaper/CNCF_cloud-native-security-whitepaper-May2022-v2.pdf)

### Recommended Courses

- [Kubernetes Security Essentials (LFS260)](https://training.linuxfoundation.org/training/kubernetes-security-essentials-lfs260/)
- [Securing Kubernetes (LFS482)](https://training.linuxfoundation.org/training/securing-kubernetes-lfs482/)

### Practice Resources

- [Kubernetes Security Best Practices](https://kubernetes.io/docs/concepts/security/security-checklist/)
- [OWASP Kubernetes Security Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/Kubernetes_Security_Cheat_Sheet.html)

## Quick Navigation

- [01 - Overview of Cloud Native Security](./01-cloud-native-security-overview.md)
- [02 - Kubernetes Cluster Component Security](./02-cluster-component-security.md)
- [03 - Kubernetes Security Fundamentals](./03-kubernetes-security-fundamentals.md)
- [04 - Kubernetes Threat Model](./04-kubernetes-threat-model.md)
- [05 - Platform Security](./05-platform-security.md)
- [06 - Compliance and Security Frameworks](./06-compliance-security-frameworks.md)
- [Sample Practice Questions](./sample-questions.md)

## Exam Tips

1. **Understand the 4Cs of Cloud Native Security** - Cloud, Cluster, Container, Code
2. **Know RBAC thoroughly** - Roles, ClusterRoles, RoleBindings, ClusterRoleBindings
3. **Study Network Policies** - Ingress and egress rules
4. **Understand Pod Security** - Security contexts, Pod Security Standards
5. **Review common attack vectors** - STRIDE, MITRE ATT&CK for Kubernetes
6. **Time management** - 90 seconds per question on average

## Registration

[Register for KCSA Exam](https://training.linuxfoundation.org/certification/kubernetes-and-cloud-native-security-associate-kcsa/)
